...
export class ChartLineWidgetComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private pendingUpdate?: { labels: string[]; datasets: any[] };
  ...
  private applyUpdate(u: { labels: string[]; datasets: any[] }): void {
    if (!this.chart) { this.pendingUpdate = u; return; }
    this.chart.data.labels = u.labels;
    this.chart.data.datasets = u.datasets;
    this.chart.update();
  }
  ...
  private loadData(): void {
    ...
    this.signalSvc.getAggregations(f?.periodType ?? 'DAY', f?.lastN ?? 30).subscribe({
      next: r => {
        const aggs = r.data ?? [];
        const labels = aggs.map(a => new Date(a.periodStart).toLocaleDateString());
        const datasets = [
          { label: 'Total',    data: aggs.map(a => a.totalCount),    borderColor: '#3b82f6', tension: 0.3, fill: false },
          { label: 'Critical', data: aggs.map(a => a.criticalCount), borderColor: '#ef4444', tension: 0.3, fill: false },
          { label: 'Resolved', data: aggs.map(a => a.resolvedCount), borderColor: '#10b981', tension: 0.3, fill: false }
        ];
        this.applyUpdate({ labels, datasets });
