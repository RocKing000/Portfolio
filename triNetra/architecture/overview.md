...
└── Research Ecosystem
    └── Insight Platforms
        ...
        └── Energy Insight Platform         ← research phase
        └── Manufacturing Insight Platform  ← research phase
        └── Infrastructure Insight Platform ← research phase
...
Most AI governance and assessment tools are built as monolithic products: one tool, one domain, one customer type. The limitation of that model is that it produces a product whose scope is defined by its first commercial use case. As the domain evolves, the product either overextends into territory it was not built for, or becomes obsolete as new requirements emerge.
...
What is domain-specific to each platform:

- The domain manifest library and templates
- The regulatory alignment mapping
- The intelligence domain structure (Organisation / Alignment / Portfolio configurations vary by platform)
- The epistemic priors and utility bounds calibrated to the domain
