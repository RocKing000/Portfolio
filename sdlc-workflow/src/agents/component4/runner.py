"""Component 4 execution runner — builds AgentContext and drives the LangGraph."""
...
from .graph import build_component4_graph


async def run_component4(
    ...
    graph = build_component4_graph(ctx)
        ...
        for _, state_update in event.items():
            if session_id in session_registry:
                ...
                if state_update.get("safe_mode"):
                    reg["status"] = "SafeMode"
                if "active_track" in state_update:
                    reg["current_track"] = state_update["active_track"]
